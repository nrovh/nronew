...
//    public static final int OPTION_PORATA = 508;
    ...
    public static final int NANG_CAP_SKH_HD = 527;
    ...
    public void showInfoCombine(Player player, int[] index) {
        ...
        switch (player.combineNew.typeCombine) {
                ...
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                    if (player.combineNew.itemsCombine.size() == 1) {
                        ...
                        if (item != null && item.isNotNullItem() &&   item.quantity >= 7) {
                            ...
                            this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Không thể thực hiện !!!", "Đóng");
                        }
                    } else {
            ...
            case NANG_CAP_BONG_TAI:
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (bongTai != null && manhVo != null && manhVo.quantity >= 9999 && bongTai.template.id == 454) {
                        ...
                        player.combineNew.goldCombine = 1000000000;
                        player.combineNew.gemCombine = 3000;
            ...
            case NANG_CAP_SKH_VIP:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.npcthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy đưa ta 1 món hủy diệt và 2 món SKH ngẫu nhiên", "Đóng");
                    return;
                }
            ...
            case NANG_CAP_SKH_HD:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.npcthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy đưa ta 1 món thiên Sứ và 1 món SKH Thần Linh", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 3) {
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTS()).count() < 1) {
                        this.npcthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu đồ Thiên Sứ", "Đóng");
                        return;
                    }
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSKHTL()).count() < 1 ) {
                        this.npcthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu đồ Thần Linh kích hoạt ", "Đóng");
                        return;
                    }
                    Item dangusac = player.combineNew.itemsCombine.get(2);
                    if (dangusac.isNotNullItem() && dangusac.template.id == 674 && dangusac.quantity >= 1) {

                        String npcSay = "|2|Con có muốn đổi các món nguyên liệu ?\n|7|"
                                + "Và nhận được "  + " kích hoạt VIP tương ứng\n"
                                + "|1|Cần " + Util.numberToMoney(COST) + " vàng";
            ...
            case RANDOM_SKH:
                if (player.combineNew.itemsCombine.isEmpty()) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy đưa ta 2 món Thần linh", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 2) {
...
//                break;    
    ...
    public void openSKHHD(Player player) {
        // 1 thiên sứ + 1 món kích hoạt TL-- món đầu kh làm gốc
        if (player.combineNew.itemsCombine.size() != 3) {
        ...
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSKHTL()).count() != 1) {
            Service.getInstance().sendThongBao(player, "Thiếu đồ thần linh kích hoạt");
            return;
        }
        Item dangusac = player.combineNew.itemsCombine.get(2);
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            ...
            List<Item> itemSKH = player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isSKHTL()).collect(Collectors.toList());
            ...
            if (player.gender == 3 || itemTS.template.type == 4) {
                itemId = Manager.radaSKHHD[Util.nextInt(0, 1)];
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.radaSKHHD[2];
                }
            } else {
                itemId = Manager.doSKHHD[player.gender][itemTS.template.type][Util.nextInt(0, 1)];
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.doSKHHD[player.gender][itemTS.template.type][2];
                }
            }
    ...
    public void openSKHVIP(Player player) {
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            ...
            if (player.gender == 3 || itemTS.template.type == 4) {
                ...
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.radaSKHVip[2];
                }
            } else {
                itemId = Manager.doSKHVip[player.gender][itemTS.template.type][Util.nextInt(0, 2)];
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.doSKHVip[player.gender][itemTS.template.type][2];
                }
            }
            int skhId = ItemService.gI().randomSKHId(player.gender);
            Item item;
    ...
    public void randomskh(Player player) {
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            ...
            if (player.gender == 3 || montldau.template.type == 4) {
                itemId = Manager.radaSKHThuong[Util.nextInt(0, 4)];
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.radaSKHThuong[4];
                }
            } else {
                itemId = Manager.doSKHThuong[player.gender][montldau.template.type][Util.nextInt(0, 4)];
                if (player.getSession().bdPlayer > 0 && Util.isTrue(1, (int) (100 / player.getSession().bdPlayer))) {
                    itemId = Manager.doSKHThuong[player.gender][montldau.template.type][4];
                }
            }
            int skhId = ItemService.gI().randomSKHId(player.gender);
...
////                
    ...
    private String getTextTopTabCombine(int type) {
        switch (type) {
            ...
            case RANDOM_SKH:
                return "Ta sẽ chuyển hóa \n 2 món Thần linh không cần thiết\n thành SKH!";
            case PHAP_SU_HOA:
