...
//                        } else
                        if (Util.kituvip(name) == false) {
                            Service.getInstance().sendThongBaoOK(session, "Tên nhân vật không được chứa ký tự VIP");
                        }else if (Util.tenadmin(name) == false) {
                            Service.getInstance().sendThongBaoOK(session, "Tên nhân vật không được đặt như vậy !");
                        } else {
    ...
    public void login2(MySession session, Message msg) {
       Service.getInstance().switchToRegisterScr(session);
        ...
        //Service.getInstance().sendThongBaoOK(session, "Vui lòng liên hệ admin để đăng ký tài khoản !");
    }
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Ngọc Rồng online server Văn Hóa\n"
                    + "Nhiệm vụ đầu tiên của bạn là di chuyển\n"
                    + "Bạn hãy di chuyển nhân vật theo mũi tên chỉ hướng");
        } else {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Ngọc rồng online server Văn Hóa\n"
                    + " Để hiểu rõ về cơ chế cũng như xem Giftcode Máy chủ Server Văn Hóa vui lòng bấm vào mục Chức năng -> Thông báo\n"
...
//        }
    ...
    private void sendThongBaoServer(Player player) {
        Service.getInstance().sendThongBaoFromAdmin(player, "|5| Ngọc rồng Văn Hóa\n|6| Chào mừng bạn đến với Ngọc rồng Online\n"
