...
public class ServerNotify extends Thread {
        ...
        while (!Maintenance.isRuning) {
            try {
                ...
                if (Util.canDoWithTime(this.lastTimeGK, 12000)) {
                    sendThongBaoBenDuoi("Chào mừng đến với Ngọc rồng VH!");
    ...
    public void sendNotifyTab(Player player) {
        ...
        try {
            ...
            msg.writer().writeUTF("Thông tin Nro VH");
            msg.writer().writeUTF("  "
                    + "\n  Nạp thẻ vui lòng liên hệ admin");
            ...
            msg.writer().writeUTF("\tCÁC LỆNH CHO MEMBER"
                    ...
                    + "\n- x n : Tăng tốc độ game (n) là số mà bạn muốn tăng tốc ví dụ : x 3 chỉ áp dụng vs pb pc"
            );
            
            msg.writer().writeShort(2);
            msg.writer().writeUTF("GIFTCODE");
             msg.writer().writeUTF("\tCÁC GIFTCODE FREE CHO MEMBER"
                    + "\n- vhchaomung : nhập tẹt ra "
                    + "\n- vhtanthu : nhập tẹt ra "
                    + "\n- Hiện chưa có gift code nào cả"
            );
