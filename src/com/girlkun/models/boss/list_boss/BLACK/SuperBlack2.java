...
public class SuperBlack2 extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        // kiếm z  
        if (Util.isTrue(1, 5)) {         
            ItemMap ct = Util.ratiItem(this.zone,865, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 27), plKill.id);
            // option cho cải trang 
            // sd
            ct.options.add(new Item.ItemOption(50, Util.nextInt(5, 15)));
            //hp
            ct.options.add(new Item.ItemOption(77, Util.nextInt(5, 20)));
            //ki
            ct.options.add(new Item.ItemOption(103, Util.nextInt(5, 20)));
            // hsd
        ...
        if ((Util.isTrue(1,10))) { // ti le rot do tl la 2/10
                ...
                it1=Util.ratiItemC(this.zone, Manager.itemIds_CUI[randomDo1], 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 18), plKill.id);
                Random splrd = new Random();
                int rdspl = splrd.nextInt(7) + 1;
                if (Util.isTrue(6, 10)) {
