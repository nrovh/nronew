...
public class CoolerGold extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        ItemMap itnew = new ItemMap(this.zone, 702, 1, this.location.x+30, this.location.y, -1);
        Service.getInstance().dropItemMap(this.zone, itnew);
        
        if(Util.isTrue(9, 10)){
        ItemMap it = new ItemMap(this.zone, 2048, 1, this.location.x-50 , this.zone.map.yPhysicInTop(this.location.x,this.location.y - 24), plKill.id);
        Service.getInstance().dropItemMap(this.zone, it);
        }
         // cải trang 
            //ItemMap ct = new ItemMap(this.zone, 20, 1, this.location.x - 10, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
            ItemMap ct = Util.ratiItem(this.zone,878, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x, this.location.y - 27), plKill.id);
            // option cho cải trang 
            // sd
            ct.options.add(new Item.ItemOption(50, Util.nextInt(15, 25)));
            //hp
            ct.options.add(new Item.ItemOption(77, Util.nextInt(15, 25)));
            //ki
            ct.options.add(new Item.ItemOption(103, Util.nextInt(15, 25)));
            // 
            ct.options.add(new Item.ItemOption(5, Util.nextInt(10, 20)));
            // hsd
