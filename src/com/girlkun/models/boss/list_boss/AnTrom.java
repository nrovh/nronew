...
public class AnTrom extends Boss {
    ...
    private static final long timeChangeMap = 500000; // thời gian đổi map 1 lần
    ...
    public void reward(Player plKill) {
       ...
       // for (byte i = 0; i < 2; i++) {
        ItemMap it2 = new ItemMap(this.zone, 722, 1, this.location.x , this.zone.map.yPhysicInTop(this.location.x,this.location.y - 24),  plKill.id);
        Service.getInstance().dropItemMap(this.zone, it2);
       // }
       
        if(Util.isTrue(5, 10)){
        ItemMap it = new ItemMap(this.zone, 2048, 1, this.location.x +40, this.zone.map.yPhysicInTop(this.location.x,this.location.y - 24), plKill.id);
